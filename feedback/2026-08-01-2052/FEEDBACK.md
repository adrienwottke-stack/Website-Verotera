# Kundenfeedback - V2 - Review ready for Aly

**Runde:** 2026-08-01-2052  
**Offene Punkte:** 95 auf 6 Seiten  
**Davon mit Antwort im Thread:** 1 (zuerst lesen!)  

## Arbeitsanweisung

- Seite fuer Seite abarbeiten, nicht nach Nummer springen.
- **Ein Commit pro Punkt:** `feedback(2026-08-01-2052): #NR kurzbeschreibung`
- **Screenshot vor der Umsetzung anschauen.** Der Pfad steht bei jedem Punkt. Der Text allein sagt oft nicht, welches Element gemeint ist.
- **Punkte mit "ANTWORT" zuerst lesen.** Die Antwort aus dem Thread kann den Wunsch einschraenken oder komplett aufheben. Im Zweifel: nicht umsetzen.
- `mobile`/`tablet` im Titel = der Kommentar kam aus dieser Ansicht. Aenderung gehoert in den Breakpoint, nicht global.
- Enthaelt ein Punkt fertigen Text in DE und EN, wird **beides** eingepflegt - nicht nur die deutsche Fassung.
- Erledigte Punkte hier abhaken (`[ ]` -> `[x]`) und mitcommitten.
- **Nicht raten.** Unklare Punkte ueberspringen und unten unter "Rueckfragen" eintragen.
- Nicht pushen, nicht deployen.

## Seiten in dieser Runde

- `https://website-verotera.vercel.app/solutions/agentic-ai-engineering` - 50 Punkte
- `https://website-verotera.vercel.app/` - 34 Punkte
- `https://website-verotera.vercel.app/solutions/wbg-power-modules` - 5 Punkte
- `https://website-verotera.vercel.app/solutions/rack-power-distribution` - 3 Punkte
- `https://website-verotera.vercel.app/resources/patents` - 2 Punkte
- `https://website-verotera.vercel.app/solutions/technology-spotlight-gallium-nitride` - 1 Punkt

---

## Seite `https://website-verotera.vercel.app/solutions/agentic-ai-engineering` (50 Punkte)

https://website-verotera.vercel.app/solutions/agentic-ai-engineering
### [x] #111 - desktop (1604px) - Thorsten Sigges

> auch hier "AI-Enhanced WBG Semiconductor Engineering" einfügen

- **Screenshot:** `feedback/2026-08-01-2052/screens/111.jpg`
- **Erstellt:** 2026-07-27 12:20
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118596/

### [ ] #112 - desktop (1604px) - Thorsten Sigges

> auch hier "AI-Enhanced WBG Semiconductor Engineering" einfügen

- **Screenshot:** `feedback/2026-08-01-2052/screens/112.jpg`
- **Erstellt:** 2026-07-27 12:21
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118602/

### [x] #113 - desktop (1604px) - Thorsten Sigges

> Sub-Headline
> "Die Begründung entsteht aus Regeln und Standards, nicht im Sprachmodell – jede Bewertung ist auf ihre Herleitung zurückführbar.“ (DE)
> 
> "The justification comes from rules and standards, not from the language model - every assessemnt traceable to its derivation." (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/113.jpg`
- **Erstellt:** 2026-07-27 12:48
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118744/

### [x] #114 - desktop (1604px) - Thorsten Sigges

> Eyebrow
> „Vom Kontext zum Konzept“ (DE)
> „From Context to Concept“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/114.jpg`
- **Erstellt:** 2026-07-27 13:14
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118863/

### [x] #115 - desktop (1604px) - Thorsten Sigges

> Headline (Frage)
> „Wie wird aus einer Anforderung eine belegte Technologieentscheidung?“ (DE)
> „How does a requirement become an evidence-based technology decision?“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/115.jpg`
- **Erstellt:** 2026-07-27 13:14
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118864/

### [x] #116 - desktop (1604px) - Thorsten Sigges

> Text
> „Die Technologiebewertung läuft in vier klar abgegrenzten Schritten ab: vom strukturierten Kontext über die abgeleiteten Bewertungskriterien und die Bewertung der Optionen samt Risiken bis zum System Design Concept. Der Ablauf ist iterativ und in jedem Schritt nachvollziehbar. Die Entscheidungsverantwortung bleibt beim Menschen.“ (DE)
> „The technology assessement runs in four clearly defined steps: from structured context, through the derived evaluation criteria and the assessment of the options and their risks, to the System Design Concept. The process is iterative and traceable at every step. Decision responsibility remains with the engineer.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/116.jpg`
- **Erstellt:** 2026-07-27 13:15
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118866/

### [ ] #117 - desktop (1604px) - Thorsten Sigges

> Process-Flow 
> Kontext erfassen –> Kriterien ableiten –> Optionen & Risiken –> System Design Concept (DE)
> Understand the context –> Derive criteria –> Options & Risks –> System Design Concept (EN)
> 
> Hinweis: "Process-flow" design-technisch etwas aufwerten (Adrien: Hast Du eine Idee?)

- **Screenshot:** `feedback/2026-08-01-2052/screens/117.jpg`
- **Erstellt:** 2026-07-27 13:24
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118899/

### [x] #118 - desktop (1604px) - Thorsten Sigges

> Header
> „Entscheidungen brauchen Nachvollziehbarkeit“ (DE)
> „Decisions need traceability“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/118.jpg`
- **Erstellt:** 2026-07-27 13:56
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119027/

### [x] #119 - desktop (1604px) - Thorsten Sigges

> Eyebrow
> „Human-in-the-Loop“ (DE/EN)
> 
> ... bleibt, sowohl in DE als auch in EN Version

- **Screenshot:** `feedback/2026-08-01-2052/screens/119.jpg`
- **Erstellt:** 2026-07-27 13:57
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119028/

### [x] #120 - desktop (1604px) - Thorsten Sigges

> Text
> „Wie liefern die Grundlage, nicht das Urteil. Das System trifft keine Technologieentscheidung – es ist nicht dafür ausgelegt. Es liefert bewertete Optionen, offengelegte Annahmen und eine Risikoübersicht: Entscheidungsgrundlagen, keine Entscheidung. Die Wahl trifft der Ingenieur, der sie verantwortet – und weil jede Bewertung auf ihre Regeln und Quellen zurückführbar ist, kann er sie prüfen, hinterfragen und vor einem Review vertreten.“ (DE)
> 
> „We provide the basis, not the decision. The system makes no technology decision – it is not built to. It delivers assessed options, disclosed assumptions and risk overview. The choice is made by the engineer, who is accountable for it – and because every assessment is traceable to ist rules and sources, it can be examined, challenged and defended in review.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/120.jpg`
- **Erstellt:** 2026-07-27 13:57
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119029/

### [x] #121 - desktop (1604px) - Thorsten Sigges

> entfernen

- **Screenshot:** `feedback/2026-08-01-2052/screens/121.jpg`
- **Erstellt:** 2026-07-27 13:58
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119031/

### [x] #122 - desktop (1604px) - Thorsten Sigges

> bleibt

- **Screenshot:** `feedback/2026-08-01-2052/screens/122.jpg`
- **Erstellt:** 2026-07-27 13:58
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119032/

### [x] #123 - desktop (1604px) - Thorsten Sigges

> Neuer Text: 
> „Collaborative Intelligence bedeutet nicht, dem Menschen Arbeit abzunehmen, sondern ihm eine Entscheidung zu geben, die er verantworten kann.“ (DE)
> „Collaborative Intelligence does not mean taking work off people´s hand – it means giving them a decision they can stanf behind.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/123.jpg`
- **Erstellt:** 2026-07-27 13:58
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119035/

### [x] #124 - desktop (1604px) - Thorsten Sigges

> entfernen

- **Screenshot:** `feedback/2026-08-01-2052/screens/124.jpg`
- **Erstellt:** 2026-07-27 13:58
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119036/

### [x] #125 - desktop (1604px) - Thorsten Sigges

> „Engineering Core“ (DE/EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/125.jpg`
- **Erstellt:** 2026-07-27 14:16
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119116/

### [x] #126 - desktop (1604px) - Thorsten Sigges

> Ein neuro-symbolischer Kern – auditierbar von Grund auf. (DE)
> 
> A neurosymbolic core – auditable by design. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/126.jpg`
- **Erstellt:** 2026-07-27 15:28
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119573/

### [x] #127 - desktop (1604px) - Thorsten Sigges

> Der Kern besteht aus fünf Verantwortungsbereichen, die ein Prinzip teilen: Interpretation und Herleitung sind getrennt. Das Sprachmodell versteht den Kontext, aber die nachvollziehbare Begründung entsteht im Wissensgraph und in den Regeln – nicht im Modell verborgen. (DE)
> 
> The core consists of five areas of responsibility that share one principle: interpretation and derivation are kept apart. The language model understands the context, but the traceable justification is built in the knowledge graph and the rules – not hidden inside the model. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/127.jpg`
- **Erstellt:** 2026-07-27 15:29
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119580/

### [ ] #128 - desktop (1604px) - Thorsten Sigges

> 01 — Orchestrierung
> Verantwortlich für den Ablauf: zerlegt die Aufgabe in Teilschritte, leitet jeden Schritt an den zuständigen Bereich und führt den Zustand des Workflows – welcher Schritt läuft, was offen ist, was entschieden wurde. Sie koordiniert, ohne selbst zu bewerten. (DE)
> 
> 01 — Orchestration
> Responsible for the flow: breaks the task into steps, routes each step to the relevant area, and maintains the state of the workflow – which step is running, what is open, what has been decided. It coordinates without assessing anything itself. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/128.jpg`
- **Erstellt:** 2026-07-27 15:30
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119581/

### [ ] #129 - desktop (1604px) - Thorsten Sigges

> 02 — Wissensgraph
> Das auditierbare Substrat des Kerns: Domänenwissen, Regeln und Normen als strukturierter, relationaler Graph statt loser Dokumente. Hier liegt die nachvollziehbare Herleitung – referenzierbar und präziser als reine Embedding-Similarity. (DE)
> 
> 02 — Knowledge Graph
> The auditable substrate of the core: domain knowledge, rules and standards held as a structured, relational graph rather than loose documents. This is where the traceable derivation lives – referenceable and more precise than embedding similarity alone. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/129.jpg`
- **Erstellt:** 2026-07-27 15:30
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119585/

### [ ] #130 - desktop (1604px) - Thorsten Sigges

> 03 — Interpretation
> Interpretiert und normalisiert die Eingaben, legt implizite Annahmen und Zielkonflikte offen und plant die mehrstufige Bewertung. Das Modell versteht den Kontext; die Herleitung selbst liefert der Wissensgraph. (DE)
> 
> 03 — Interpretation
> Interprets and normalizes the inputs, surfaces implicit assumptions and conflicting objectives, and plans the multi-step assessment. The model understands the context; the derivation itself comes from the knowledge graph. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/130.jpg`
- **Erstellt:** 2026-07-27 15:31
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119591/

### [ ] #131 - desktop (1604px) - Thorsten Sigges

> 04 — Absicherung
> Belastbarkeit statt Plausibilität: prüft die Ergebnisse gegen Evidenz, hinterlegte Regeln und definierte Prüfkriterien. Was die Prüfung nicht besteht, wird markiert und eskaliert – keine ungeprüfte Ausgabe. (DE)
> 04 — Assurance
> Substantiated, not merely plausible: checks the results against evidence, encoded rules and defined acceptance criteria. Anything that fails the check is flagged and escalated – no unchecked output. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/131.jpg`
- **Erstellt:** 2026-07-27 15:32
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119596/

### [ ] #132 - desktop (1604px) - Thorsten Sigges

> Verantwortungsbereich 5 hinzufügen!
> 05 — Agent
> Der WBG-Agent führt die Bewertung aus: wendet die Regeln an, prüft Constraints und durchläuft den Zyklus aus Planung, Werkzeugaufruf, Prüfung und Iteration. Jeder Schritt bleibt an Orchestrierung und Regelwerk gebunden. (DE)
> 
> 05 — Agent
> The WBG agent carries out the assessment: applies the rules, checks constraints, and runs the cycle of planning, tool call, verification and iteration. Every step stays bound to the orchestration and the rule set. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/132.jpg`
- **Erstellt:** 2026-07-27 15:33
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119606/

### [x] #133 - desktop (1604px) - Thorsten Sigges

> "Time-to-Market" (DE/EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/133.jpg`
- **Erstellt:** 2026-07-27 15:41
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119662/

### [x] #134 - desktop (1604px) - Thorsten Sigges

> "Time-to-Market entscheidet sich in der Konzeptphase – nicht am Ende." (DE)
> 
> "Time-to-market is decided in the concept phase – not at the end." (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/134.jpg`
- **Erstellt:** 2026-07-27 15:49
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119717/

### [x] #135 - desktop (1604px) - Thorsten Sigges

> "Je später eine Technologieentscheidung revidiert werden muss, desto teurer wird sie – in Redesign-Schleifen, Requalifizierung und verlorener Zeit. Der Engineering Core sichert die SiC/GaN-Entscheidung schon früh ab: nachvollziehbar begründet, mit offengelegten Risiken. Das verkürzt nicht einen einzelnen Arbeitsschritt, sondern verhindert die Umwege, die die Time-to-Market wirklich kosten." (DE)
> 
> "The later a technology decision has to be reversed, the more it costs – in redesign loops, requalification and lost time. The Engineering Core de-risks the SiC/GaN decision early: with traceable justification and disclosed risks. It doesn't shorten a single task – it prevents the detours that truly cost time-to-market."

- **Screenshot:** `feedback/2026-08-01-2052/screens/135.jpg`
- **Erstellt:** 2026-07-27 15:50
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119723/

### [ ] #136 - desktop (1604px) - Thorsten Sigges

> 01 — Technologieklassen
> Si, SiC und GaN als strukturierte Klassen mit ihren Eigenschaften und Einsatzgrenzen – nicht als Datenblatt, sondern als vergleichbare, referenzierbare Objekte.
> 
> 01 — Technology Classes
> Si, SiC and GaN as structured classes with their properties and application limits – not a datasheet, but comparable, referenceable objects.

- **Screenshot:** `feedback/2026-08-01-2052/screens/136.jpg`
- **Erstellt:** 2026-07-27 16:06
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119818/

### [ ] #137 - desktop (1604px) - Thorsten Sigges

> 02 — Funktionsblöcke
> Wiederkehrende Bausteine der Leistungselektronik – Gate-Treiber, Kühlpfad, Filter – als definierte Blöcke mit Schnittstellen und Randbedingungen.
> 
> 02 — Functional Blocks
> Recurring building blocks of power electronics – gate driver, cooling path, filter – as defined blocks with interfaces and constraints.

- **Screenshot:** `feedback/2026-08-01-2052/screens/137.jpg`
- **Erstellt:** 2026-07-27 16:06
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119823/

### [ ] #138 - desktop (1604px) - Thorsten Sigges

> 03 — Topologien
> Schaltungstopologien als eigenständige Objekte, mit ihren Spannungsstressfaktoren und Eignungsgrenzen – die Brücke zwischen Technologiewahl und Architektur.
> 
> 03 — Topologies
> Circuit topologies as objects in their own right, with their voltage stress factors and suitability limits – the bridge between technology choice and architecture.

- **Screenshot:** `feedback/2026-08-01-2052/screens/138.jpg`
- **Erstellt:** 2026-07-27 16:06
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119828/

### [ ] #139 - desktop (1604px) - Thorsten Sigges

> 04 — Regeln
> Kodierte Ingenieurslogik: Bedingungen, die Technologien, Topologien und Anforderungen verknüpfen – prüfbar, versioniert, nachvollziehbar.
> 
> 04 — Rules
> Encoded engineering logic: conditions that link technologies, topologies and requirements – checkable, versioned, traceable.

- **Screenshot:** `feedback/2026-08-01-2052/screens/139.jpg`
- **Erstellt:** 2026-07-27 16:07
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119831/

### [ ] #140 - desktop (1604px) - Thorsten Sigges

> 05 — Parametrische Modelle
> Physikalische Zusammenhänge als parametrische Objekte – getrennt von den Regeln, sodass Berechnung und Entscheidungslogik nicht vermischt werden.
> 
> 05 — Parametric Models
> Physical relationships as parametric objects – kept separate from the rules, so that calculation and decision logic are not intermixed.

- **Screenshot:** `feedback/2026-08-01-2052/screens/140.jpg`
- **Erstellt:** 2026-07-27 16:07
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119835/

### [ ] #141 - desktop (1604px) - Thorsten Sigges

> 06 — Anforderungen
> Zielgrößen und Randbedingungen als strukturierte Objekte – normalisiert, auf Zielkonflikte geprüft und mit den übrigen Bausteinen verknüpft.
> 
> 06 — Requirements
> Target values and constraints as structured objects – normalized, checked for conflicting objectives, and linked to the other building blocks.

- **Screenshot:** `feedback/2026-08-01-2052/screens/141.jpg`
- **Erstellt:** 2026-07-27 16:07
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119841/

### [x] #142 - desktop (1604px) - Thorsten Sigges

> Diesen Bereich löschen

- **Screenshot:** `feedback/2026-08-01-2052/screens/142.jpg`
- **Erstellt:** 2026-07-27 16:21
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119980/

### [x] #143 - desktop (1604px) - Thorsten Sigges

> Auch diesen Bereich löschen

- **Screenshot:** `feedback/2026-08-01-2052/screens/143.jpg`
- **Erstellt:** 2026-07-27 16:22
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119981/

### [x] #144 - desktop (1604px) - Thorsten Sigges

> Structured Requirement Object (DE/EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/144.jpg`
- **Erstellt:** 2026-07-27 16:23
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12119991/

### [x] #145 - desktop (1604px) - Thorsten Sigges

> Physik begründet, Evidenz validiert, Ingenieursregeln führen – und Collaborative Intelligence macht das Ergebnis nachvollziehbar. (DE)
> 
> Engineering truth is established by physics, validated by evidence, governed by engineering rules, and communicated through Collaborative Intelligence. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/145.jpg`
- **Erstellt:** 2026-07-27 16:24
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12120004/

### [x] #146 - desktop (1604px) - Thorsten Sigges

> Ein maschinenlesbares Artefakt, das die bewertete Entscheidung samt Herleitung, Annahmen und Risiken trägt – in kanonischer Form, aus der sich Exportsichten wie ReqIF oder SysML ableiten. (DE)
> 
> A machine-readable artifact that carries the assessed decision together with its derivation, assumptions and risks – in a canonical form from which export views such as ReqIF or SysML are derived. (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/146.jpg`
- **Erstellt:** 2026-07-27 16:25
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12120009/

### [x] #147 - desktop (1904px) - Thorsten Sigges

> An dieser Stelle bitte 6 Feld-Paare einfügen... im gleichen Design wie aktuelle, d.h. Grafik-Elemente Rechtecke mit abgerundeten Kanten. Dazu ein Beispiel eine SRO - als maschinenlesbarer Export (JSON) auch in einem Rechteck.

- **Screenshot:** `feedback/2026-08-01-2052/screens/147.jpg`
- **Erstellt:** 2026-07-28 08:19
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126559/

### [x] #148 - desktop (1904px) - Thorsten Sigges

> SRO als maschinenlesbarer Export
> 
> SRO as a machine-readable export

- **Screenshot:** `feedback/2026-08-01-2052/screens/148.jpg`
- **Erstellt:** 2026-07-28 08:21
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126562/

### [x] #149 - desktop (1904px) - Thorsten Sigges

> Die Entscheidung als vollständiges, verlustfreies Objekt.
> 
> The decision as a complete, lossless object.

- **Screenshot:** `feedback/2026-08-01-2052/screens/149.jpg`
- **Erstellt:** 2026-07-28 08:26
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126574/

### [x] #150 - desktop (1904px) - Thorsten Sigges

> Hier die 6 Elemente einfügen Headline plus Text - in der richtigen Reihenfolge 01-06:

- **Screenshot:** `feedback/2026-08-01-2052/screens/150.jpg`
- **Erstellt:** 2026-07-28 08:28
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126582/

### [x] #151 - desktop (1904px) - Thorsten Sigges

> 01
> Anforderungen
> Was gefordert ist, strukturiert und normalisiert.
> Erfasste Felder: Use Case, Vin, Vout, Pout, Isolation, Ziel-Wirkungsgrad, Leistungsdichte, Kühlkonzept, Normen, Umgebung.
> 
> 01
> Requirements
> What is required, structured and normalized.
> Captured fields: use case, Vin, Vout, Pout, isolation, target efficiency, power density, cooling concept, standards, environment.

- **Screenshot:** `feedback/2026-08-01-2052/screens/151.jpg`
- **Erstellt:** 2026-07-28 08:29
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126586/

### [x] #152 - desktop (1604px) - Thorsten Sigges

> 02
> Kandidaten & Herleitung
> Die bewerteten Technologiepfade – jeder mit der Begründung, warum er wie abschneidet, zurückführbar auf Regeln und Modelle.
> Je Kandidat: Technologiepfad, Bewertung pro Kriterium, Trade-offs, Verweis auf Regel und Modell.
> 
> 02
> Candidates & Derivation
> The assessed technology paths – each with the reasoning for how it performs, traceable to rules and models.
> Per candidate: technology path, assessment per criterion, trade-offs, reference to rule and model.

- **Screenshot:** `feedback/2026-08-01-2052/screens/152.jpg`
- **Erstellt:** 2026-07-28 08:30
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126589/

### [x] #153 - desktop (1604px) - Thorsten Sigges

> 03
> Offene Entscheidungen
> Punkte, an denen mehrere Pfade zulässig bleiben – bewusst nicht aufgelöst, sondern dem Ingenieur vorgelegt.
> Je Eintrag: konkurrierende Pfade, auslösendes Zielkonflikt-Paar, Grund der Nicht-Auflösung.
> 
> 03
> Open Decisions
> Points where several paths remain valid – deliberately left unresolved and put to the engineer.
> Per entry: competing paths, the conflicting objectives behind it, reason for non-resolution.

- **Screenshot:** `feedback/2026-08-01-2052/screens/153.jpg`
- **Erstellt:** 2026-07-28 08:31
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126590/

### [x] #154 - desktop (1604px) - Thorsten Sigges

> 04
> Eskalation an den Menschen
> Stellen, an denen Unsicherheit oder Konflikt eine Schwelle überschreiten und eine menschliche Entscheidung verlangen.
> Je Eintrag: betroffenes Kriterium, Art der Unsicherheit, überschrittene Schwelle, geforderte Prüfung.
> 
> 04
> Escalation to the Human
> Points where uncertainty or conflict crosses a threshold and calls for a human decision.
> Per entry: affected criterion, type of uncertainty, threshold crossed, review required.

- **Screenshot:** `feedback/2026-08-01-2052/screens/154.jpg`
- **Erstellt:** 2026-07-28 08:31
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126591/

### [x] #155 - desktop (1604px) - Thorsten Sigges

> 05
> Annahmen
> Was angenommen wurde, wo Daten fehlten – explizit, nicht implizit.
> Je Annahme: getroffene Annahme, betroffene Größe, Grund (fehlende Daten / Vereinfachung).
> 
> 05
> Assumptions
> What was assumed where data was missing – made explicit, not left implicit.
> Per assumption: the assumption made, the quantity affected, reason (missing data / simplification).

- **Screenshot:** `feedback/2026-08-01-2052/screens/155.jpg`
- **Erstellt:** 2026-07-28 08:32
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126593/

### [x] #156 - desktop (1604px) - Thorsten Sigges

> 06
> Provenance
> Woher jede Aussage stammt – Quelle, Regel, Modellversion, Zeitpunkt.
> Je Aussage: Quelle, angewandte Regel, Modellversion, Stand/Zeitpunkt.
> 
> 
> 06
> Provenance
> Where every statement comes from – source, rule, model version, point in time.
> Per statement: source, rule applied, model version, timestamp.

- **Screenshot:** `feedback/2026-08-01-2052/screens/156.jpg`
- **Erstellt:** 2026-07-28 08:33
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126594/

### [x] #157 - desktop (1604px) - Thorsten Sigges

> Grafik beibehalten... anderer Inhalt!
> oben nur SRO... darunter:
> {
>   "requirement_id": "<string>",
>   "use_case": "<string>",
>   "operating_point": {
>     "Vin": "<voltage>",
>     "Vout": "<voltage>",
>     "Pout": "<power>",
>     "isolation": "<bool>"
>   },
>   "candidates": [
>     {
>       "technology_path": "<enum: Si | SiC | GaN>",
>       "assessment": "<qualitative | semi-quantitative>",
>       "trade_offs": ["<string>"],
>       "derived_from": { "rule": "<rule_ref>", "model": "<model_ref>" }
>     }
>   ],
>   "open_decisions": ["<string>"],
>   "escalation": {
>     "criterion": "<string>",
>     "reason": "<string>",
>     "requires": "human_review"
>   },
>   "assumptions": ["<string>"],
>   "provenance": {
>     "source": "<ref>",
>     "rule_version": "<semver>",
>     "timestamp": "<ISO-8601>"
>   }
> }
> 
> Bildunterschriften:
> Bildunterschrift (DE): Kanonische Form – Basis für Exportsichten (ReqIF, SysML).
> Bildunterschrift (EN): Canonical form – the basis for export views (ReqIF, SysML).

- **Screenshot:** `feedback/2026-08-01-2052/screens/157.jpg`
- **Erstellt:** 2026-07-28 08:35
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126600/

### [x] #158 - desktop (1604px) - Thorsten Sigges

> Statement (DE): Die kanonische Form ist maßgeblich. SysML und ReqIF sind Exportsichten. Herleitung, Eskalation und Provenance sind in keinem Zielformat nativ abbildbar – nur die kanonische Form trägt sie vollständig.
> Statement (EN): The canonical form is authoritative. SysML and ReqIF are export views. Derivation, escalation and provenance cannot be represented natively in any target format – only the canonical form carries them in full.

- **Screenshot:** `feedback/2026-08-01-2052/screens/158.jpg`
- **Erstellt:** 2026-07-28 08:36
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126601/

### [x] #159 - desktop (1604px) - Thorsten Sigges

> Auch diesen Bereich entfernen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/159.jpg`
- **Erstellt:** 2026-07-28 08:39
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126603/

### [ ] #160 - desktop (1604px) - Thorsten Sigges

> Der gesamte "Content" für diesen Bereich befindet sich auch hier: https://verotera.sharepoint.com/:w:/s/TeamWebsite/IQBOez3wIohyQ6oMm9jK5YwWAT6tuC5FjZda5xTSBBDYvN8?e=z6BPhw

- **Screenshot:** `feedback/2026-08-01-2052/screens/160.jpg`
- **Erstellt:** 2026-07-28 08:41
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126610/

---

## Seite `https://website-verotera.vercel.app/` (34 Punkte)

https://website-verotera.vercel.app/
### [ ] #75 - desktop (1582px) - Aly Mashaly

> Stil der Bilder auf dem Landingpage ist nicht einheitlich

- **Screenshot:** `feedback/2026-08-01-2052/screens/75.jpg`
- **Erstellt:** 2026-07-24 12:38
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108274/

### [x] #76 - desktop (1582px) - Aly Mashaly

> Ich würde hier die Zeit des Wechsels redzieren

- **Screenshot:** `feedback/2026-08-01-2052/screens/76.jpg`
- **Erstellt:** 2026-07-24 14:04
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108477/

### [x] #77 - desktop (1582px) - Aly Mashaly

> die Pfeile waren für mich nicht sofort sichtbar. Am besten die Pfeile nach oben schieben, etwas vergrößen und rechts und links vom Bild platzieren. Sodass sie sichtbarer werden.

- **Screenshot:** `feedback/2026-08-01-2052/screens/77.jpg`
- **Erstellt:** 2026-07-24 14:05
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108482/

### [ ] #78 - desktop (1904px) - Thorsten Sigges

> hier bitte die neuen Images aus "Shared Resources" ... aa_data_center_new, aa_emobilit_new, aa_hydrogen_new, aa_renewable_new und aa_robotics_new... einfügen. Sie sind den Bildern weiter unten (Hover) im Design angepasst worden. (auf Wunsch von Aly)

- **Screenshot:** `feedback/2026-08-01-2052/screens/78.jpg`
- **Erstellt:** 2026-07-24 14:19
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108537/

### [x] #79 - desktop (1904px) - Thorsten Sigges

> (Bereich: Hero-Header) "AI-Enhanced WBG Semiconductor Engineering" (DE und EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/79.jpg`
- **Erstellt:** 2026-07-24 14:40
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108629/

### [x] #80 - desktop (1904px) - Thorsten Sigges

> (Bereich Sub-Header) "Collaborative Intelligence - das System begründet, der Mensch entscheidet und verantwortet" (DE) "Collaborative Intelligence - the system provides the reasoning, the engineer decides and is accountable" (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/80.jpg`
- **Erstellt:** 2026-07-24 14:40
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108635/

### [x] #81 - desktop (1904px) - Thorsten Sigges

> (Bereich: "Value Proposition") "Technologiepfade früh bewerten, die Begründung inklusive - aus Regeln, Normen und Annahmen" (DE) "Evaluate technology paths early - reasoning included, grounded in rules, standards and assumptions" (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/81.jpg`
- **Erstellt:** 2026-07-24 14:42
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108643/

### [x] #82 - desktop (1582px) - Aly Mashaly

> Hier wird Englisch und Deutsch dargestellt

_Hinfaellig: betrifft dieselbe Sektion wie #87 ("Collaborative Intelligence" / "Powered by People
and Precision"). Thread-Antwort von Thorsten Sigges (2026-07-28) gilt auch hier: bewusst englisch,
bleibt so. Keine Code-Aenderung._

- **Screenshot:** `feedback/2026-08-01-2052/screens/82.jpg`
- **Erstellt:** 2026-07-24 15:04
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108750/

### [ ] #83 - desktop (1582px) - Aly Mashaly

> Diese Botschaft muss geändert bzw. angepasst werden

- **Screenshot:** `feedback/2026-08-01-2052/screens/83.jpg`
- **Erstellt:** 2026-07-24 15:05
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12108754/

### [x] #84 - desktop (1582px) - Aly Mashaly

> der Satz vom Smart PDU kann komplett weg

- **Screenshot:** `feedback/2026-08-01-2052/screens/84.jpg`
- **Erstellt:** 2026-07-24 17:13
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12109995/

### [x] #85 - desktop (1582px) - Aly Mashaly

> der zweite Satz (Vom Chip bis zum System....) kann auch weg.

- **Screenshot:** `feedback/2026-08-01-2052/screens/85.jpg`
- **Erstellt:** 2026-07-24 17:17
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110039/

### [x] #86 - desktop (1582px) - Aly Mashaly

> Dieser Teil wird nochmal im nächsten Teil unten wiederholt. Entweder hier weg lassen oder unten entfernen

- **Screenshot:** `feedback/2026-08-01-2052/screens/86.jpg`
- **Erstellt:** 2026-07-24 17:53
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110366/

### [x] #87 - desktop (1582px) - Aly Mashaly

> Das muss auf deutsch für diese Seite sein.

_Hinfaellig durch die Thread-Antwort unten: bleibt englisch. Keine Code-Aenderung._

> [!WARNING]
> **ANTWORT IM THREAD:** (2026-07-28 08:45) Thorsten Sigges: Das wurde bewusst in englisch ausgeführt - da es sich auf deutsch nicht wirklich gut anhört - ist ein durchaus akzeptables Stilmittel und findet sich an anderen Stellen auch

- **Screenshot:** `feedback/2026-08-01-2052/screens/87.jpg`
- **Erstellt:** 2026-07-24 17:53
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110371/

### [ ] #88 - desktop (1582px) - Aly Mashaly

> Dieser Abschnitt evtl. Komplett entfernen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/88.jpg`
- **Erstellt:** 2026-07-24 17:55
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110386/

### [x] #89 - desktop (1582px) - Aly Mashaly

> Ich weiß nicht, ob wieder IEA hier auf unsere Webseite zetieren dürfen. ICh würde es sicherheitshalbe rausnehmen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/89.jpg`
- **Erstellt:** 2026-07-24 17:56
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110396/

### [ ] #90 - desktop (1582px) - Aly Mashaly

> Diese Zahlen passen m.E. eher auf Präsentation und nicht auf der Webseite.

- **Screenshot:** `feedback/2026-08-01-2052/screens/90.jpg`
- **Erstellt:** 2026-07-24 17:59
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12110412/

### [x] #98 - desktop (1604px) - Thorsten Sigges

> Eyebrow (kleine Schrift)
> „Früher. Belegbar. Belastbar.“ (DE)
> „Earlier. Documented. Dependable.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/98.jpg`
- **Erstellt:** 2026-07-27 09:54
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12117985/

### [x] #99 - desktop (1904px) - Thorsten Sigges

> Headline
> „Technologieentscheidungen in der Konzeptphase“ (DE)
> „Technology decisions in the concept phase.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/99.jpg`
- **Erstellt:** 2026-07-27 09:56
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12117990/

### [x] #100 - desktop (1904px) - Thorsten Sigges

> Text
> „In der sehr frühen Konzeptphase werden Systemanforderungen und Randbedingungen normalisiert, implizite Annahmen und Zielkonflikte identifiziert und SiC/GaN-Technologiepfade gegen abgeleitete Kriterien bewertet – qualitativ bis semi-quantitativ, mit expliziter Unsicherheitskennzeichnung. Ausgabe ist ein System Design Concept samt Trade-off-Bewertung, Annahmen- und Risikoübersicht. Die Entscheidungsverantwortung verbleibt beim Ingenieur.“
> 
> „In the early concept phase, the impact of decisions is highest and the available data is most limited. System requirements and constraints are normalized, implicit assumptions and conflicting objectives are identified, and SiC/GaN technology paths are assessed against derived criteria – qualitatively to semi-qualitatively, with explicit flagging of uncertainty. The output is aSystem Design Concept with a trade-off assessment and an overview of assumptions and risks. Decision responsibility remeians with the engineer.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/100.jpg`
- **Erstellt:** 2026-07-27 09:56
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12117991/

### [x] #101 - desktop (1904px) - Thorsten Sigges

> „AI-Enhanced Engineering“  statt "Agentic AI"

- **Screenshot:** `feedback/2026-08-01-2052/screens/101.jpg`
- **Erstellt:** 2026-07-27 09:57
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12117995/

### [x] #102 - desktop (1604px) - Thorsten Sigges

> Wir gehen hier nicht auf die einzelnen Phasen ein - die Grafik soll aufzeigen, dass wir in einer sehr frühen Phase (die fünf linken Phasen) begleiten - die rechte V-Seite kann optional visuell etwas zurück genommen werden (oder "out of scope" betiteln)

- **Screenshot:** `feedback/2026-08-01-2052/screens/102.jpg`
- **Erstellt:** 2026-07-27 10:02
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118015/

### [x] #103 - desktop (1604px) - Thorsten Sigges

> Eyebrow
> Collaborative Intelligence – ein neuro-symbolischer Ansatz (DE)
> Collaborative Intelligence – a neurosymbolic approach (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/103.jpg`
- **Erstellt:** 2026-07-27 10:52
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118229/

### [x] #104 - desktop (1604px) - Thorsten Sigges

> Headline
> „Die KI erweitert das Ingenieururteil – Von der Empfehlung zur belegten Entscheidungsgrundlage – nachvollziehbar und reproduzierbar.“ (DE)
> „AI extends engineering judgment – turning a recommendation into a documented basis for decisions: traceable and reproducible.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/104.jpg`
- **Erstellt:** 2026-07-27 10:52
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118231/

### [x] #105 - desktop (1604px) - Thorsten Sigges

> Header
> „Human-in-the-Loop by design“ (DE/EN)
> Hinweis: Eyebrow „Smarter Design Decisions“ ganz streichen!!!

- **Screenshot:** `feedback/2026-08-01-2052/screens/105.jpg`
- **Erstellt:** 2026-07-27 10:56
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118248/

### [x] #106 - desktop (1604px) - Thorsten Sigges

> „VEROTERA strukturiert die folgenreichste Entscheidung der frühen Entwicklungsphase: die Wide-Bandgap-Technologieauswahl – bewertet, begründet und auf ihre Quellen zurückführbar.“ (DE)
> „VEROTERA brings structure to the most consequential decision of the early development phase: the Wide-Bandgap technology choice – assessed, justified and traceable to ist sources.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/106.jpg`
- **Erstellt:** 2026-07-27 11:25
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118369/

### [x] #107 - desktop (1604px) - Thorsten Sigges

> Nur 4 Punkte auflisten:
> •	Weniger teure und zeitintensive Redesign-Schleifen durch belastbare Frühentscheidungen
> •	SiC/GaN-Technologiepfade nachvollziehbar bewertet – nicht aus dem Bauch
> •	Annahmen und Risiken explizit dokumentiert, die Herleitung aus Regeln und Standards nachvollziehbar – kein Blackbox-Modell
> •	On-Premise-Betrieb: eigne Daten und IP bleiben im Haus, DSGVO-konform
> 
> •	Fewer costly redesign loops through dependable early decisons
> •	SiC/GaN technology paths assessed transparently – not from good feeling
> •	Assumptions and risks documented explicity, the derivation traceable to rules and standards – no black-box-model
> •	On-premise operation: your data and IP stay in-house, GDPR-compliant.

- **Screenshot:** `feedback/2026-08-01-2052/screens/107.jpg`
- **Erstellt:** 2026-07-27 11:26
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118371/

### [x] #108 - desktop (1604px) - Thorsten Sigges

> „So wird aus einer riskanten Frühentscheidung eine dokumentierte, überprüfbare Grundlage – die einem Review standhält“ (DE)
> 
> „A risky early decison becomes a documented, verifable basis – one that holds up in review.“ (EN)

- **Screenshot:** `feedback/2026-08-01-2052/screens/108.jpg`
- **Erstellt:** 2026-07-27 11:26
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118372/

### [ ] #109 - desktop (1604px) - Thorsten Sigges

> Hier Image aa-engineering-core - aus "Team Website" hochladen https://verotera.sharepoint.com/:i:/s/TeamWebsite/IQBoibCcOW7eTruqz1EgdDvVAVdkYqgA60ibOsvW7j4XcTo?e=1AQ5KG

- **Screenshot:** `feedback/2026-08-01-2052/screens/109.jpg`
- **Erstellt:** 2026-07-27 12:16
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118568/

### [x] #110 - desktop (1604px) - Thorsten Sigges

> Hier "AI-Enhanced WBG Semiconductor Engineering" einfügen

- **Screenshot:** `feedback/2026-08-01-2052/screens/110.jpg`
- **Erstellt:** 2026-07-27 12:19
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12118587/

### [x] #161 - desktop (1604px) - Thorsten Sigges

> Komplett löschen... wir gehen auf das Thema im Bereich "Engineering Core" ein - an dieser Stelle nur der Fokus auf WBG

- **Screenshot:** `feedback/2026-08-01-2052/screens/161.jpg`
- **Erstellt:** 2026-07-28 08:50
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12126646/

### [x] #162 - desktop (1688px) - Aly Mashaly

> Support, Technische Unterstützung und Kontakt. m. E. reicht nur eine der drei aus.

- **Screenshot:** `feedback/2026-08-01-2052/screens/162.jpg`
- **Erstellt:** 2026-07-28 10:10
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12127076/

### [x] #163 - desktop (1688px) - Aly Mashaly

> Patente muss man nicht extra schreiben.

- **Screenshot:** `feedback/2026-08-01-2052/screens/163.jpg`
- **Erstellt:** 2026-07-28 10:14
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12127106/

### [ ] #164 - desktop (1688px) - Aly Mashaly

> Dieser Teil sollte anderes dargestellt werden

- **Screenshot:** `feedback/2026-08-01-2052/screens/164.jpg`
- **Erstellt:** 2026-07-31 10:12
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12154935/

### [ ] #166 - desktop (1264px) - Thorsten Sigges

> Hier bitte eine neues Image einfügen (verbesserte Wafer/Dies Darstellung) siehe immages/aa-wafer-dies-sic.png

- **Screenshot:** `feedback/2026-08-01-2052/screens/166.jpg`
- **Erstellt:** 2026-07-31 11:16
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12155157/

---

## Seite `https://website-verotera.vercel.app/solutions/wbg-power-modules` (5 Punkte)

https://website-verotera.vercel.app/solutions/wbg-power-modules
### [ ] #91 - desktop (1582px) - Aly Mashaly

> Das Bildstil hier soll wie beim Landingpage sein.

- **Screenshot:** `feedback/2026-08-01-2052/screens/91.jpg`
- **Erstellt:** 2026-07-24 22:44
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112630/

### [ ] #92 - desktop (1582px) - Aly Mashaly

> Text ändern

- **Screenshot:** `feedback/2026-08-01-2052/screens/92.jpg`
- **Erstellt:** 2026-07-24 22:45
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112633/

### [ ] #93 - desktop (1688px) - Aly Mashaly

> Bild ändern

- **Screenshot:** `feedback/2026-08-01-2052/screens/93.jpg`
- **Erstellt:** 2026-07-24 22:46
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112637/

### [ ] #94 - desktop (1688px) - Aly Mashaly

> die steilichen Pins am Modul rechts rausnehmen

- **Screenshot:** `feedback/2026-08-01-2052/screens/94.jpg`
- **Erstellt:** 2026-07-24 22:46
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112641/

### [ ] #95 - desktop (1688px) - Aly Mashaly

> Die Verlinkung mit LinkedIn funktioniert nicht

- **Screenshot:** `feedback/2026-08-01-2052/screens/95.jpg`
- **Erstellt:** 2026-07-24 22:47
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112644/

---

## Seite `https://website-verotera.vercel.app/solutions/rack-power-distribution` (3 Punkte)

https://website-verotera.vercel.app/solutions/rack-power-distribution
### [x] #165 - desktop (1406px) - Aly Mashaly

> Künstliche Intelligenz verändert nahezu alle Branchen in einem noch nie dagewesenen Tempo – und die Rechenzentren, die diese Revolution antreiben, wachsen ebenso rasant. Während das Training früherer, kleinerer KI-Modelle noch dem Jahresverbrauch von rund 100 bis 200 Haushalten entsprach, ziehen moderne KI-Trainingscluster heute dauerhaft 100 bis 300 Megawatt Leistung – das entspricht dem Strombedarf von zehntausenden Haushalten. Einzelne neue Rechenzentrumsprojekte erreichen sogar Leistungen von 300 bis 500 Megawatt, was dem Verbrauch mittelgroßer Städte mit mehreren hunderttausend Einwohnern gleichkommt.
> 
> Da KI-Workloads in allen Sektoren zunehmen, wird der kumulierte Einfluss auf die globalen Energiesysteme immer deutlicher. Große Rechenzentrumsbetreiber sind für einen Großteil dieses Wachstums verantwortlich und investieren weltweit Milliardenbeträge in neue, KI-optimierte Infrastruktur. Die Internationale Energieagentur prognostiziert, dass der weltweite Stromverbrauch von Rechenzentren von 415 Terawattstunden im Jahr 2024 auf rund 945 Terawattstunden bis 2030 ansteigen wird – eine Verdopplung, die vor allem durch den Ausbau KI-spezifischer Rechenkapazitäten getrieben wird.

- **Screenshot:** `feedback/2026-08-01-2052/screens/165.jpg`
- **Erstellt:** 2026-07-31 10:52
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12155072/

### [ ] #167 - desktop (1900px) - Aly Mashaly

> hier würde ich eher das Blockdiagramm aus der PPT ersetzen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/167.jpg`
- **Erstellt:** 2026-07-31 11:43
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12155220/

### [x] #168 - desktop (1688px) - Aly Mashaly

> Ich würde diesen Teil komplett erstmal rausnehmen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/168.jpg`
- **Erstellt:** 2026-07-31 11:49
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12155243/

---

## Seite `https://website-verotera.vercel.app/resources/patents` (2 Punkte)

https://website-verotera.vercel.app/resources/patents
### [ ] #96 - desktop (1688px) - Aly Mashaly

> Stimmt das wirklich, dass wir die KI-Systems Engineering patentieren?

- **Screenshot:** `feedback/2026-08-01-2052/screens/96.jpg`
- **Erstellt:** 2026-07-24 22:48
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112652/

### [ ] #97 - desktop (1688px) - Aly Mashaly

> Ich würde die Landingpage zum Thema Patente erstmal weglassen. Das ist zu früh, diese hier zu veröffentlichen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/97.jpg`
- **Erstellt:** 2026-07-24 22:50
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12112658/

---

## Seite `https://website-verotera.vercel.app/solutions/technology-spotlight-gallium-nitride` (1 Punkt)

https://website-verotera.vercel.app/solutions/technology-spotlight-gallium-nitride
### [ ] #169 - desktop (1688px) - Aly Mashaly

> die zwei Seiten von GaN und SiC rausnehmen.

- **Screenshot:** `feedback/2026-08-01-2052/screens/169.jpg`
- **Erstellt:** 2026-07-31 11:53
- **In Pastel:** https://usepastel.com/link/onxnqep4/comment/12155255/

---

## Rueckfragen an den Kunden

_(hier eintragen, was nicht eindeutig war - kommt als Reply zurueck ins Pastel-Canvas)_

### Seite `/` (Landingpage)

**Warten auf Bilddateien** - die genannten Dateien liegen weder in `public/images/` noch sonst
im Repo, nur als SharePoint-Link im Kommentar:

- **#78** - `aa_data_center_new`, `aa_emobilit_new`, `aa_hydrogen_new`, `aa_renewable_new`,
  `aa_robotics_new` fuer die 5 Kacheln der Mission-Sektion.
- **#109** - `aa-engineering-core` fuer die Agentic-Ecosystem-Sektion.
- **#166** - `aa-wafer-dies-sic.png` fuer die WBG-Modulloesungen-Sektion.
- **#75** - "Bildstil nicht einheitlich" haengt an #78: solange die neuen Bilder fehlen, gibt es
  keine Stil-Referenz. Zusatzfrage: welcher Stil gilt als verbindlich, und wer liefert die Bilder
  fuer die uebrigen Sektionen - ihr oder ich?

**Fehlende Zielvorgabe** - Kommentar nennt kein Ergebnis, jede Umsetzung waere geraten:

- **#83** - "Diese Botschaft muss geaendert bzw. angepasst werden" (Karte 1 "Vom Chip bis zum
  System" / FULL STACK). Kein Ersatztext geliefert. Haengt mit #88 zusammen, das dieselbe Karte
  evtl. ganz loeschen will.
- **#88** - "Dieser Abschnitt evtl. Komplett entfernen": Rahmen sitzt nur auf dem Titel von
  Karte 1, der Text spricht von "Abschnitt". Gemeint ist nur diese eine Karte, das ganze
  Karten-Grid (aktuell 3 Karten, nach #161) oder die komplette Sektion?
- **#90** - "Diese Zahlen passen m.E. eher auf Praesentation": Rahmen umschliesst nur die Karte
  "100+ Entwickelte Produkte", der Text sagt "diese Zahlen" (Plural). Alle vier Kennzahlen raus,
  nur die markierte, oder die ganze Sektion inkl. Gruender-Zitat?
- **#164** - "Dieser Teil sollte anderes dargestellt werden" (Advanced Packaging): Wie soll der
  Block stattdessen aussehen?

**Umgesetzt unter Annahme - bitte gegenpruefen:**

- **#89** - IEA-Quellenangabe nur auf der Landingpage entfernt. Die IEA steht auch in
  `/solutions/rack-power-distribution` und `/applications/automotive-emobility` - dort nicht
  angefasst, weil nicht Teil dieses Abschnitts. Achtung: der neue Text aus #165 zitiert die IEA
  ausdruecklich. Die Rechtefrage muss fuer alle Stellen gleich beantwortet werden.
- **#102** - Variante "rechte V-Seite visuell zurueckgenommen" gewaehlt (statt "out of scope"
  betiteln). Die drei Kacheln im unteren Trunk sind unveraendert, weil der Kommentar explizit
  "rechte V-Seite" sagt - soll der Trunk mit zurueckgenommen werden?
- **#162** - Von den drei genannten Eintraegen ist "Technische Unterstuetzung" entfernt; "Support"
  bleibt als Spaltenueberschrift, "Kontakt" als Link. Grund: beide Links zeigten auf dieselbe
  Route `/contacts`.
- **#86** - Die obere Dublette (Abschlusssatz der Mission-Sektion) ist entfernt, die Aufzaehlung
  der Anwendungsfelder im Abschnitt darunter bleibt stehen.

---

## Rueckfragen ausserhalb der Landingpage

Fertig formulierte Kommentare zum Einfuegen ins Pastel-Canvas, je Punkt in DE und EN.
Grundlage: PLAN.md (nur Punkte, die dort als ENTSCHEIDUNG / ASSET_FEHLT bzw. mit Rueckfrage
markiert sind) plus der jeweilige Screenshot.

### Seite `/solutions/agentic-ai-engineering`

#### #112 - Eyebrow und H1 waeren wortgleich

**DE:**
> #111 ist umgesetzt – die Pill über der Überschrift heißt jetzt „AI-Enhanced WBG Semiconductor
> Engineering". #112 verlangt denselben Text für die H1 direkt darunter; dann stünde derselbe
> Begriff zweimal untereinander. Wie soll es aussehen?
> A) Die Pill behält den Begriff, die H1 bekommt einen eigenen Titel – welchen? (aktuell:
> „Agentische KI im Systems Engineering")
> B) Die H1 bekommt den Begriff, die Pill wird ein kurzes Label (z. B. „Lösungen" oder
> „AI Systems Engineering").
> C) Bewusst identisch – dann setze ich es 1:1 so um.

**EN:**
> #111 is done – the pill above the headline now reads "AI-Enhanced WBG Semiconductor
> Engineering". #112 asks for the same wording in the H1 directly below it, which would print
> the same term twice. Which way should it go?
> A) The pill keeps the term and the H1 gets its own title – which one? (currently:
> "Agentic AI in Systems Engineering")
> B) The H1 takes the term and the pill becomes a short label (e.g. "Solutions" or
> "AI Systems Engineering").
> C) Deliberately identical – then I implement it exactly as specified.

#### #133-#141 - Sektionsheader passt nicht zu den sechs Karten (kommentiert an #136)

**DE:**
> Die Sektion trägt jetzt den Header aus #133–#135 („Time-to-Market" / „Time-to-Market
> entscheidet sich in der Konzeptphase – nicht am Ende.") und darunter die sechs Karten aus
> #136–#141 (Technologieklassen, Funktionsblöcke, Topologien, Regeln, Parametrische Modelle,
> Anforderungen). Inhaltlich sind das die Bausteine des Wissensgraphen, nicht Time-to-Market.
> Wie ist es gemeint?
> A) Zwei Sektionen: „Time-to-Market" bleibt als reine Aussage (Header + Text, ohne Karten), die
> sechs Bausteine bekommen eine eigene Sektion – dafür brauche ich Eyebrow, Headline und
> Einleitungstext in DE und EN.
> B) Eine Sektion – dann bitte einen Header, der zu den sechs Bausteinen passt.
> C) So gewollt, bleibt wie es ist.
> Zusatzfrage: Unter den Karten stehen noch drei Labels aus der alten Fassung
> („Human-in-the-Loop Gates", „Audit-Ready Traceability", „Continuous Knowledge Capture").
> Bleiben, ersetzen oder raus?

**EN:**
> The section now carries the header from #133–#135 ("Time-to-Market" / "Time-to-market is
> decided in the concept phase – not at the end.") with the six cards from #136–#141 below it
> (technology classes, functional blocks, topologies, rules, parametric models, requirements).
> In substance those are the knowledge-graph building blocks, not time-to-market. How is it
> meant?
> A) Two sections: "Time-to-Market" stays a pure statement (header + text, no cards) and the six
> building blocks get their own section – for that I need an eyebrow, headline and intro text
> in German and English.
> B) One section – then please supply a header that fits the six building blocks.
> C) Intentional, leave as is.
> Follow-up: three labels from the old version are still sitting below the cards
> ("Human-in-the-Loop Gates", "Audit-Ready Traceability", "Continuous Knowledge Capture").
> Keep, replace or remove?

#### #117 - Process-Flow design-technisch aufwerten

**DE:**
> Die vier Labels sind gesetzt. Offen ist der Zusatz „Process-Flow design-technisch etwas
> aufwerten". Drei Vorschläge – welcher soll es werden?
> A) (meine Empfehlung) Nummerierte Prozessleiste: 01–04 als Cyan-Kreise auf einer durchgehenden
> Linie, Label darunter, auf dem Handy untereinander.
> B) Vier Karten mit Icon, Nummer und Label, verbunden durch Cyan-Pfeile – visuell identisch zu
> den Karten der Engineering-Core-Sektion weiter unten.
> C) Es bleibt die Pill-Reihe, nur größer, mit Nummern und kräftigerem Cyan-Pfeil.

**EN:**
> The four labels are in place. What is still open is the note "make the process flow visually
> stronger". Three options – which one?
> A) (My recommendation) Numbered process bar: 01–04 as cyan circles on a continuous line, labels
> underneath, stacked vertically on mobile.
> B) Four cards with icon, number and label, connected by cyan arrows – visually identical to
> the cards in the Engineering Core section further down.
> C) Keep the pill row, just larger, with numbers and a stronger cyan arrow.

#### #160 - Word-Dokument in SharePoint nicht zugaenglich

**DE:**
> Ich komme an das verlinkte Word-Dokument nicht heran – SharePoint verlangt einen
> Verotera-Login. Bitte entweder als .docx oder .pdf schicken oder den Zugriff freigeben.
> Und: Der Kommentar hängt oben im Seitenkopf, ohne einen markierten Bereich – bezieht sich
> „dieser Bereich" auf die gesamte Agentic-AI-Seite oder auf eine bestimmte Sektion?

**EN:**
> I cannot open the linked Word document – SharePoint requires a Verotera login. Please either
> send it as .docx or .pdf or grant access. Also: the comment sits at the top of the page
> without a marked element – does "this area" refer to the entire Agentic AI page or to one
> specific section?

#### #113, #116, #120, #123 - Tippfehler in den gelieferten Texten

**DE:**
> In den gelieferten Texten waren ein paar Vertipper. Ich habe sie stillschweigend korrigiert:
> #120 DE „Wie liefern die Grundlage" → „Wir liefern die Grundlage"
> #120 EN „traceable to ist rules" → „its rules"
> #113 EN „every assessemnt traceable" → „every assessment is traceable"
> #116 EN „technology assessement" → „technology assessment"
> #123 EN „people´s hand" → „people's hands" und „a decision they can stanf behind" →
> „stand behind"
> Bitte kurz bestätigen. Und generell für die nächsten Runden: Soll ich Tippfehler still
> korrigieren oder Texte wörtlich übernehmen und zurückmelden?

**EN:**
> The supplied texts contained a few typos. I corrected them silently:
> #120 DE "Wie liefern die Grundlage" → "Wir liefern die Grundlage"
> #120 EN "traceable to ist rules" → "its rules"
> #113 EN "every assessemnt traceable" → "every assessment is traceable"
> #116 EN "technology assessement" → "technology assessment"
> #123 EN "people´s hand" → "people's hands" and "a decision they can stanf behind" →
> "stand behind"
> Please confirm. And as a general rule for the coming rounds: should I correct typos silently
> or copy the text verbatim and flag it back to you?

### Seite `/solutions/wbg-power-modules`

#### #91 - "Bildstil wie auf der Landingpage"

**DE:**
> Markiert ist die helle Strichgrafik (Fahrzeug mit SiC-/GaN-Modulen) in der Sektion
> „Technologie- & Systempartner". „Bildstil wie auf der Landingpage" reicht mir als Vorgabe noch
> nicht: Auf der Landingpage steht der Bildstil selbst zur Diskussion (#75), und die dort
> gewünschten neuen Dateien (aa_data_center_new, aa_emobilit_new usw.) liegen mir nicht vor.
> Bitte ein konkretes Referenzbild benennen, das den Zielstil zeigt – und wer liefert den Ersatz
> für diese Grafik?

**EN:**
> The marked element is the light line drawing (vehicle with SiC/GaN modules) in the
> "Technology & System Partner" section. "Same image style as the landing page" is not yet
> enough to work from: the landing page style is itself under discussion (#75), and the new
> files requested there (aa_data_center_new, aa_emobilit_new etc.) have not reached me.
> Please name one concrete reference image that shows the target style – and who supplies the
> replacement for this graphic?

#### #92 - "Text aendern" ohne Ersatztext

**DE:**
> Markiert ist der Absatz „VEROTERA begleitet Kunden von den frühesten Entwicklungsphasen an …"
> unter „Wide-Bandgap at the Core". „Text ändern" ohne Ersatztext kann ich nicht raten. Bitte
> den neuen Text in DE und EN liefern – oder kurz sagen, was stört (zu allgemein, inhaltlich
> falsch, zu lang), dann mache ich einen Vorschlag.

**EN:**
> The marked element is the paragraph "VEROTERA supports customers from the earliest development
> phases …" under "Wide-Bandgap at the Core". "Change text" without a replacement leaves me
> guessing. Please supply the new text in German and English – or tell me briefly what is wrong
> with it (too generic, factually off, too long) and I will draft a proposal.

#### #93 - "Bild aendern" ohne Zielbild

**DE:**
> Markiert ist das dunkelblaue Render (Wafer unter Laser) neben „Die Brücke zwischen Nachfrage
> und Innovation". „Bild ändern" sagt nicht, wohin. Was soll das neue Bild zeigen, und kommt die
> Datei von euch oder soll ich einen Vorschlag machen? Bitte zusammen mit #91 beantworten,
> damit beide Bilder im selben Stil landen.

**EN:**
> The marked element is the dark blue render (wafer under a laser) next to "The bridge between
> demand and innovation". "Change image" does not say what to. What should the new image show,
> and will the file come from you or should I propose one? Please answer together with #91 so
> both images end up in the same style.

#### #95 - LinkedIn-Link tot

**DE:**
> Der LinkedIn-Link im Footer zeigt auf https://www.linkedin.com/company/verotera – unter dieser
> Adresse existiert keine Seite. Bitte die korrekte URL der Unternehmensseite schicken. Falls es
> noch keine gibt: Soll ich den LinkedIn-Block vorerst ausblenden? Der Link steht im Footer,
> also auf jeder Seite.

**EN:**
> The LinkedIn link in the footer points to https://www.linkedin.com/company/verotera – there is
> no page at that address. Please send the correct company page URL. If there is none yet:
> should I hide the LinkedIn block for now? The link lives in the footer, so it appears on
> every page.

### Seite `/solutions/rack-power-distribution`

#### #165 - IEA-Zitierung widerspricht #89

**DE:**
> Der neue Text ist eingepflegt, die englische Fassung habe ich übersetzt. Er nennt ausdrücklich
> „Die Internationale Energieagentur" mit der Prognose 415 → 945 TWh. Gleichzeitig habe ich nach
> #89 die IEA-Quellenangabe auf der Landingpage entfernt, weil unklar war, ob VEROTERA die IEA
> zitieren darf. Beides zusammen ist widersprüchlich – was gilt?
> A) Die IEA darf genannt werden → ich setze die Quellenangabe auf der Landingpage wieder ein.
> B) Die IEA wird nirgends genannt → ich formuliere den Satz hier neutral um (z. B. „Prognosen
> gehen davon aus, dass …"). Bitte die Ersatzformulierung bestätigen.
> C) Nennung im Fließtext ist in Ordnung, aber keine Report-Quellenangabe („World Energy
> Outlook 2025").
> Die Antwort betrifft auch `/applications/automotive-emobility`, dort steht die IEA ebenfalls im
> Text.

**EN:**
> The new text is in place and I translated the English version. It explicitly names "the
> International Energy Agency" with the 415 → 945 TWh projection. At the same time, following
> #89 I removed the IEA source line from the landing page because it was unclear whether VEROTERA
> may cite the IEA. The two contradict each other – which is it?
> A) The IEA may be named → I restore the source line on the landing page.
> B) The IEA is not named anywhere → I reword the sentence here neutrally (e.g. "projections
> indicate that …"). Please confirm the replacement wording.
> C) Naming it in running text is fine, but no report citation ("World Energy Outlook 2025").
> The answer also applies to `/applications/automotive-emobility`, where the IEA appears in the
> text as well.

### Seite `/resources/patents`

#### #96 - Patentaussage zum KI-Systems-Engineering

**DE:**
> Markiert ist die Unterzeile „Geschützte Innovationen … bis zum KI-gestützten Systems
> Engineering". Weiter unten steht dazu eine eigene Karte („KI-gestütztes Engineering – Methoden
> im agentischen Systems Engineering"). Gibt es in diesem Bereich tatsächlich eine Anmeldung?
> Falls nein: Aussage auf Halbleiter/Module und Packaging reduzieren oder ganz streichen?
> Falls die Seite nach #97 ohnehin verschwindet, erledigt sich der Punkt – eine kurze Antwort
> hilft trotzdem, weil der Text an anderer Stelle wiederverwendet wird.

**EN:**
> The marked element is the subline "Protected innovations … to AI-assisted systems
> engineering". Further down there is a dedicated card for it ("AI-Assisted Engineering –
> methods in agentic systems engineering"). Is there actually a filing in that area?
> If not: should the claim be reduced to semiconductors/modules and packaging, or dropped
> entirely? If the page disappears anyway under #97 the point is moot – a short answer still
> helps, because the wording is reused elsewhere.

#### #97 - Patentseite loeschen oder nur entlinken

**DE:**
> „Erstmal weglassen" lässt zwei Wege offen – bitte einen wählen:
> A) (meine Empfehlung, weil „erstmal") Die Seite bleibt bestehen, wird aber aus Navigation,
> Sitemap und Google genommen. Reversibel, keine toten Links.
> B) Die Seite wird gelöscht. Dann richte ich eine Weiterleitung auf `/resources` oder die
> Startseite ein, sonst laufen bereits indexierte Links ins Leere.
> Der Footer-Link ist mit #163 schon entfernt; übrig sind die Einträge in der Sitemap und auf der
> Site-Map-Seite.

**EN:**
> "Leave it out for now" allows two routes – please pick one:
> A) (My recommendation, because it is temporary) The page stays online but is removed from the
> navigation, the sitemap and Google. Reversible, no dead links.
> B) The page is deleted. In that case I add a redirect to `/resources` or the homepage,
> otherwise already-indexed links break.
> The footer link is already gone with #163; what remains are the entries in the sitemap and on
> the site map page.

### Seite `/solutions/technology-spotlight-gallium-nitride`

#### #169 - GaN- und SiC-Seite loeschen oder nur entlinken

**DE:**
> Der Punkt betrifft beide Spotlight-Seiten (GaN und SiC). Gleiche Frage wie bei den Patenten:
> A) Die Seiten bleiben online, aber ohne Verlinkung und aus Google genommen – reversibel.
> B) Die Seiten werden gelöscht, dann mit Weiterleitung auf `/solutions/wbg-power-modules`.
> Zusätzlich: Die beiden Seiten sind an mehreren Stellen verlinkt – im Menü „Lösungen", in der
> Lösungs-Übersicht der Startseite, auf der Site-Map und in vier News-Beiträgen. Sollen diese
> Einträge komplett verschwinden oder stattdessen auf `/solutions/wbg-power-modules` zeigen?
> Und ist das dauerhaft gemeint oder wie bei #97 nur „erstmal"?

**EN:**
> This affects both spotlight pages (GaN and SiC). Same question as for the patents page:
> A) The pages stay online but unlinked and removed from Google – reversible.
> B) The pages are deleted, with a redirect to `/solutions/wbg-power-modules`.
> In addition: the two pages are linked from several places – the "Solutions" menu, the
> solutions overview on the homepage, the site map and four news items. Should those entries
> disappear entirely or point to `/solutions/wbg-power-modules` instead? And is this meant to be
> permanent, or "for now" as in #97?
